...
TB_TempArray[TB_Counter]!=a?TB_FoundURL?(TB_NextCaption=TB_TempArray[TB_Counter],TB_NextURL=TB_TempArray[TB_Counter].href,TB_NextHTML="<span id='TB_next'>&nbsp;&nbsp;<a href='#'>"+thickboxL10n.next+"</a></span>"):(TB_PrevCaption=TB_TempArray[TB_Counter],TB_PrevURL=TB_TempArray[TB_Counter].href,TB_PrevHTML="<span id='TB_prev'>&nbsp;&nbsp;<a href='#'>"+thickboxL10n.prev+"</a></span>"):(TB_FoundURL=!0,TB_imageCount=thickboxL10n.image+" "+(TB_Counter+1)+" "+thickboxL10n.of+" "+TB_TempArray.length);TB_FirstCaption=
TB_TempArray[0];TB_FirstURL=TB_TempArray[0].href;TB_LastCaption=TB_TempArray[TB_TempArray.length-1];TB_LastURL=TB_TempArray[TB_TempArray.length-1].href}imgPreloader=new Image;imgPreloader.onload=function(){imgPreloader.onload=null;var d=c.replace(/^[^\?]+\??/,""),e=tb_parseQuery(d),g=tb_getPageSize(),d=g[0]-150,m=g[1]-150,g=e.width?Math.min(e.width*1,imgPreloader.width):imgPreloader.width,k=e.height?Math.min(e.height*1,imgPreloader.height):imgPreloader.height;if(tb_options.auto_resize)if(g>d){k=k*
(d/g);g=d;if(k>m){g=g*(m/k);k=m}}else if(k>m){g=g*(m/k);k=m;if(g>d){k=k*(d/g);g=d}}d=tb_caption(a);TB_Image="<img id='TB_Image' src='"+c+"' width='"+g+"' height='"+k+"' alt='"+d+"'/>";if(tb_options.click_img=="close"||!b&&tb_options.click_end=="close")TB_Image="<a href='' id='TB_ImageOff' title='"+thickboxL10n.close+"'>"+TB_Image+"</a>";TB_Caption="<div id='TB_caption'>"+d+"<div id='TB_secondLine'>"+TB_imageCount+TB_PrevHTML+TB_NextHTML+"</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='"+
thickboxL10n.close+"'><img src='"+tb_closeImage+"' /></a></div>";tb_options.effect_cap!="none"&&(TB_Caption="<div id='TB_CaptionBar'>"+TB_Caption+"</div>");switch(tb_options.position_cap){case "top":jQuery("#TB_window").append(TB_Caption+TB_Image).addClass("TB_imageContent TB_captionTop");break;case "bottom":jQuery("#TB_window").append(TB_Image+TB_Caption).addClass("TB_imageContent TB_captionBottom");break;case "none":jQuery("#TB_window").append(TB_Image).addClass("TB_imageContent")}(!b||!d)&&jQuery("#TB_window").addClass("TB_singleLine");
d=tb_getSize(jQuery("#TB_Image"),"margin");TB_WIDTH=d[0];TB_HEIGHT=d[1]+tb_getSize(jQuery("#TB_closeWindow"),"margin")[1];if(TB_WIDTH<e.width*1){TB_WIDTH=e.width*1;jQuery("#TB_Image").css({marginLeft:"auto",marginRight:"auto"})}TB_HEIGHT<e.height*1&&(TB_HEIGHT=e.height*1);e.modal=="true"&&jQuery("#TB_overlay").unbind();jQuery("#TB_closeWindowButton").click(tb_remove);if(TB_PrevHTML!==""){var j=function(){jQuery(document).unbind(".thickbox");jQuery("#TB_window").remove();jQuery("body").append("<div id='TB_window' class='TB_Transition'></div>");
tb_show(TB_PrevCaption,TB_PrevURL,b);return false};jQuery("#TB_prev").click(j);var n=function(){jQuery(document).unbind(".thickbox");jQuery("#TB_window").remove();jQuery("body").append("<div id='TB_window' class='TB_Transition'></div>");tb_show(TB_FirstCaption,TB_FirstURL,b);return false}}if(TB_NextHTML!==""){var o=function(){jQuery(document).unbind(".thickbox");jQuery("#TB_window").remove();jQuery("body").append("<div id='TB_window' class='TB_Transition'></div>");tb_show(TB_NextCaption,TB_NextURL,
b);return false};jQuery("#TB_next").click(o);var p=function(){jQuery(document).unbind(".thickbox");jQuery("#TB_window").remove();jQuery("body").append("<div id='TB_window' class='TB_Transition'></div>");tb_show(TB_LastCaption,TB_LastURL,b);return false}}if(b&&tb_options.click_img=="next"){var l,h,f;if(TB_NextHTML!=""){l="TB_ImageNext";h=thickboxL10n.next;f=o}else if(tb_options.click_end=="close"){l="TB_ImageClose";h=thickboxL10n.close;f=tb_remove}else if(tb_options.click_end=="loop"&&TB_PrevHTML!=
""){l="TB_ImageFirst";h=thickboxL10n.first;f=n}if(l&&h&&f){d=tb_getSize(jQuery("#TB_Image"),"margin")[1];jQuery("#TB_window").append("<div id='TB_ImageClick'><a href='' id='"+l+"' title='"+h+"' style='height:"+d+"px;'><span></span></a></div>");jQuery("#"+l).click(f)}}else if(b&&tb_options.click_img=="prev_next"){var i,q,r;if(TB_PrevHTML!=""){l="TB_ImagePrev";h=thickboxL10n.prev;f=j}if(TB_NextHTML!=""){i="TB_ImageNext";q=thickboxL10n.next;r=o}if(TB_PrevHTML=="")if(tb_options.click_end=="close"){l=
"TB_ImageClose";h=thickboxL10n.close;f=tb_remove}else if(tb_options.click_end=="loop"&&TB_NextHTML!=""){l="TB_ImageLast";h=thickboxL10n.last;f=p}if(TB_NextHTML=="")if(tb_options.click_end=="close"){i="TB_ImageClose2";q=thickboxL10n.close;r=tb_remove}else if(tb_options.click_end=="loop"&&TB_PrevHTML!=""){i="TB_ImageFirst";q=thickboxL10n.first;r=n}if(l&&h&&f||i&&q&&r){d=tb_getSize(jQuery("#TB_Image"),"margin")[1];g=link2="";l&&(h&&f)&&(g="<a href='' id='"+l+"' class='TB_ImageLeft' title='"+h+"' style='height:"+
d+"px;'></a>");i&&(q&&r)&&(link2="<a href='' id='"+i+"' class='TB_ImageRight' title='"+q+"' style='height:"+d+"px;'></a>");jQuery("#TB_window").append("<div id='TB_ImageClick'>"+g+link2+"</div>");g&&jQuery("#"+l).click(f);link2&&jQuery("#"+i).click(r);jQuery.browser.msie&&parseInt(jQuery.browser.version)<=7&&jQuery("#TB_ImageClick > a").focus(function(){this.blur()})}}jQuery(document).bind("keydown.thickbox",function(a){a.stopImmediatePropagation();e.modal!="true"&&jQuery.inArray(a.which,tb_options.keys_close)!=
-1?jQuery(document).triggerHandler("wp_CloseOnEscape",[{event:a,what:"thickbox",cb:tb_remove}])||tb_remove():jQuery.inArray(a.which,tb_options.keys_prev)!=-1||a.shiftKey&&jQuery.inArray(a.which,tb_options.keys_prev.shift)!=-1?TB_PrevHTML!=""&&j():jQuery.inArray(a.which,tb_options.keys_next)!=-1?TB_NextHTML!=""&&o():jQuery.inArray(a.which,tb_options.keys_first)!=-1?TB_PrevHTML!=""&&n():jQuery.inArray(a.which,tb_options.keys_last)!=-1&&TB_NextHTML!=""&&p();return false});b&&tb_options.wheel_img=="prev_next"&&
jQuery("#TB_window").bind("mousewheel.thickbox",function(a){a.stopImmediatePropagation();a=0;if(!event)event=window.event;event.wheelDelta?a=event.wheelDelta:event.detail&&(a=-event.detail);if(a>0){if(TB_PrevHTML!=""){j();return false}}else if(a<0&&TB_NextHTML!=""){o();return false}});tb_options.move_img&&jQuery("#TB_window").bind("mousedown.thickbox",function(a){return tb_move(a,"image")});if(tb_options.resize_img){tb_resize_init();jQuery("#TB_Resize").bind("mousedown.thickbox",function(a){return tb_resize(a,
"image")})}if(tb_options.effect_cap!="none"){jQuery("#TB_Image").css("margin",jQuery("#TB_Image").css("marginLeft"));tb_options.effect_cap=="zoom"&&jQuery("#TB_CaptionBar").css({left:"50%",marginLeft:-TB_WIDTH/2});jQuery("#TB_window").bind("mouseenter.thickbox",function(){tb_hover(jQuery("#TB_CaptionBar"),tb_options.effect_cap)}).bind("mouseleave.thickbox",function(){tb_hover(jQuery("#TB_CaptionBar"),tb_options.effect_cap,true)})}tb_position();jQuery("#TB_load").remove();(tb_options.click_img=="close"||
!b&&tb_options.click_end=="close")&&jQuery("#TB_ImageOff").click(tb_remove);tb_open()};imgPreloader.src=c}else{var s=c.replace(/^[^\?]+\??/,""),j=tb_parseQuery(s);TB_WIDTH=1*j.width||600;TB_HEIGHT=1*j.height||400;ajaxContentW=TB_WIDTH;ajaxContentH=TB_HEIGHT;var n=tb_caption(a,tb_options.ref_title);TB_Title="<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+n+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='"+thickboxL10n.close+"'><img src='"+tb_closeImage+"' /></a></div></div>";
if(-1!=c.indexOf("TB_iframe"))if(ajaxContentW+=2*("\v"=="v"?10:8),ajaxContentH+=2*("\v"=="v"?15:8),urlNoQuery=c.split(/[?&]TB_/),jQuery("#TB_iframeContent").remove(),TB_Iframe="<iframe frameborder='0' hspace='0' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(1E3*Math.random())+"' onload='tb_showIframe()' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px;'>"+thickboxL10n.noiframes+"</iframe>","true"!=j.modal)switch(tb_options.position_title){case "top":jQuery("#TB_window").append(TB_Title+
...
ajaxContentW+"px",jQuery("#TB_ajaxContent")[0].style.height=ajaxContentH+"px",jQuery("#TB_ajaxContent")[0].scrollTop=0,jQuery("#TB_ajaxWindowTitle").html(n);if(-1!=c.indexOf("TB_iframe"))TB_WIDTH+=2*("\v"=="v"?10:8),TB_HEIGHT+=2*("\v"=="v"?15:8)+tb_getSize(jQuery("#TB_title"))[1];else{var p=tb_getSize(jQuery("#TB_ajaxContent"),"padding"),t=tb_getSize(jQuery("#TB_ajaxContent"));TB_WIDTH+=p[0]-t[0];TB_HEIGHT+=p[1]-t[1]+tb_getSize(jQuery("#TB_title"))[1]}jQuery("#TB_closeWindowButton").click(tb_remove);
if(-1!=c.indexOf("TB_inline"))jQuery("#TB_ajaxContent").append(jQuery("#"+j.inlineId).children()),jQuery("#TB_window").bind("tb_unload",function(){jQuery("#"+j.inlineId).append(jQuery("#TB_ajaxContent").children())}),tb_position(),jQuery("#TB_load").remove(),tb_open(),tb_roundCorner("iframe");else if(-1!=c.indexOf("TB_iframe"))tb_position(),!("onload"in jQuery("#TB_iframeContent")[0])&&"function"!=typeof jQuery("#TB_iframeContent")[0].onload&&tb_showIframe();else{var u=-1==c.indexOf("?")?"?":"&";
jQuery("#TB_ajaxContent").load(c+=u+"random="+(new Date).getTime(),function(){tb_position();jQuery("#TB_load").remove();tb_open();tb_roundCorner("ajax")})}tb_options.move_content&&jQuery("#TB_window").bind("mousedown.thickbox",function(a){return tb_move(a,c.indexOf("TB_iframe")!=-1?"iframe":"ajax")});tb_options.resize_content&&(tb_resize_init(),jQuery("#TB_Resize").bind("mousedown.thickbox",function(a){return tb_resize(a,c.indexOf("TB_iframe")!=-1?"iframe":"ajax")}));"none"!=tb_options.effect_title&&
(jQuery("#TB_title").addClass("hover"),"zoom"==tb_options.effect_title&&jQuery("#TB_title").css({left:"50%",marginLeft:-TB_WIDTH/2}),jQuery("#TB_window").bind("mouseenter.thickbox",function(){tb_hover(jQuery("#TB_title"),tb_options.effect_title)}).bind("mouseleave.thickbox",function(){tb_hover(jQuery("#TB_title"),tb_options.effect_title,true)}));"true"!=j.modal&&jQuery(document).bind("keyup.thickbox",function(a){if(jQuery.inArray(a.which,tb_options.keys_close)!=-1){a.stopImmediatePropagation();jQuery(document).triggerHandler("wp_CloseOnEscape",
[{event:a,what:"thickbox",cb:tb_remove}])||tb_remove();return false}})}}catch(v){tb_options.debug&&alert(v)}}
...
jQuery("#TB_window").css("display","block");jQuery("#TB_title").height(jQuery("#TB_ajaxWindowTitle").height()+13)}
...
function tb_drag(a,c,b){if(1!=a.which)return!0;TB_THRESHOLD=5;MIN_WIDTH=MIN_HEIGHT=100;var d=jQuery("#TB_window").offset().left-parseInt(jQuery("#TB_window").css("marginLeft")),e=jQuery("#TB_window").offset().top-parseInt(jQuery("#TB_window").css("marginTop"));"undefined"!=typeof document.body.style.maxHeight&&"absolute"!=jQuery("#TB_window").css("position")&&(d-=jQuery(document).scrollLeft(),e-=jQuery(document).scrollTop());var s=a.clientX,j=a.clientY,n=!1,p=jQuery("#TB_window, #TB_overlay, "+("image"==
c?"#TB_window a":"#TB_closeWindowButton")),t="move"==b||"w-resize"==b||"nw-resize"==b||"sw-resize"==b,u="move"==b||"n-resize"==b||"ne-resize"==b||"nw-resize"==b,v="move"!=b&&"n-resize"!=b&&"s-resize"!=b,x="move"!=b&&"e-resize"!=b&&"w-resize"!=b;if("move"!=b){var y=jQuery("#TB_window").width(),g=jQuery("#TB_window").height(),m=jQuery("image"==c?"#TB_Image":"iframe"==c?"#TB_iframeContent":"#TB_ajaxContent"),k=m.width(),z=m.height(),w="e-resize"==b||"ne-resize"==b||"se-resize"==b?1:-1,o="s-resize"==
b||"se-resize"==b||"sw-resize"==b?1:-1;if("undefined"==typeof document.body.style.maxHeight)var A=jQuery("#TB_window").css("marginTop")}jQuery(document).bind("mousemove.thickbox",function(a,h,f){var h=h?h:a.clientX,f=f?f:a.clientY,a=h-s,i=f-j;if(!n){a=a>TB_THRESHOLD?a-TB_THRESHOLD:a<-TB_THRESHOLD?a+TB_THRESHOLD:0;i=i>TB_THRESHOLD?i-TB_THRESHOLD:i<-TB_THRESHOLD?i+TB_THRESHOLD:0;if(Math.abs(a)>0||Math.abs(i)>0){n=true;s=h;j=f;p.css("cursor",b)}}if(n){h=skipMoveY=false;if(v){f=y+a*w;if(f<MIN_WIDTH)h=
true;else{jQuery("#TB_window").width(f);m.width(k+a*w)}}if(x){f=g+i*o;if(f<MIN_HEIGHT)skipMoveY=true;else{jQuery("#TB_window").height(f);m.height(z+i*o);c=="image"&&jQuery("#TB_ImageClick > a").height(jQuery("#TB_Image").outerHeight(true));typeof document.body.style.maxHeight=="undefined"&&jQuery("#TB_window").css("marginTop",A)}}t&&!h&&jQuery("#TB_window").css("left",d+a);u&&!skipMoveY&&jQuery("#TB_window").css("top",e+i)}return false});jQuery(document).bind("mouseup.thickbox",function(a){jQuery(document).unbind("mousemove.thickbox mouseup.thickbox");
c=="iframe"&&jQuery("#TB_iframeContent").contents().unbind("mousemove.thickbox mouseup.thickbox");if(!n)return true;p.css("cursor","");var b=jQuery(a.target);if(b.is("html"))return false;b.is(".TB_ImageLeft, .TB_ImageRight")||(b=b.parent());if((a=b.data("events"))&&a.click){var d=[];jQuery.each(a.click,function(a,b){d[d.length]=b.handler||b});if(d.length>0)b.unbind("click").one("click",function(a){a.preventDefault();a.stopImmediatePropagation?a.stopImmediatePropagation():a.stopPropagation();for(a=
