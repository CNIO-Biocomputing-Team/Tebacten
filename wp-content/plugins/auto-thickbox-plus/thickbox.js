...
function tb_show(elem, url, imageGroup) {//function called when the user clicks on a thickbox link
	try {
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp' || urlType == '.webp'){//code to show images
			...
			if(imageGroup){
				...
				for (TB_Counter = 0; ((TB_Counter < TB_TempArray.length) && (TB_NextHTML === "")); TB_Counter++) {
					...
					if (!(TB_TempArray[TB_Counter] == elem)) {
						if (TB_FoundURL) {
			...
			var caption = tb_caption(elem, tb_options.ref_title);
...
function tb_open() {
	...
	jQuery("#TB_title").height(jQuery("#TB_ajaxWindowTitle").height() + 13); // TODO: workaround code for the height of multiline title (to be modified)
}
